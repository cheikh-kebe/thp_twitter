...
def twitt_tag(client)
  client.update("Salut les journalites #bonjourmonde #{handle.sample}}")
end

#5.times {|i|twitt_tag(login_twitter)}#send 5 twitts

def like_client(client)
  like = client.search("#bonjour_monde", result_type: "recent").take(25)
  
  client.favorite(like)
end

#like_client(login_twitter)

#module pour follow n profils qui ont liké le #bonjour_monde
def follow_client(client)
  follow = client.search("#bonjour_monde").take(5).collect do |tweet|
...
def stream(client_2)
  ...
  client_2.filter(track: topics.join(",")) do |object|
    puts object.text if object.is_a?(Twitter::Tweet)
  end
end

stream(login_twitter_stream)
